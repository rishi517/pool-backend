...
from lib.types import AgentRequest, AgentResponse, prebuilt_llm, State, StoreInfo
from firebase_functions import logger
from langgraph.prebuilt import create_react_agent
from langchain_core.messages import AIMessage
from tools.store_tools import get_store_details_tool, get_store_hours_tool


system_message = """You are a the specific store expert agent for Heritage Pool's customer service system. 

Minimize the number of tools you use.

Your capabilities include:
1. Getting store details for a specific store id (get_store_details_tool)
2. Getting store hours for a specific store id (get_store_hours_tool)
...
Focus only on the store details for Heritage Pool Plus.
...
def store_info_agent_node(state: State) -> Command[str]:
    try:
        ...
        logger.error(f"Error in store_info_agent_node: {str(e)}")
        raise